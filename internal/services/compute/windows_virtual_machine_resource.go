...
func windowsVirtualMachine() *pluginsdk.Resource {
	return &pluginsdk.Resource{
		...
		Schema: map[string]*pluginsdk.Schema{
			...
			// TODO: Uncomment dedicated_host_id until dedicated host is available
			/*"dedicated_host_id": {
				Type:         pluginsdk.TypeString,
				Optional:     true,
			...
			// TODO: uncomment proximity_placement_group_id if the resource is available
			/*
...
func resourceWindowsVirtualMachineDelete(d *pluginsdk.ResourceData, meta interface{}) error {
	...
	if strings.EqualFold(*existing.ProvisioningState, "failed") {
		...
		powerOffFuture, err := client.PowerOff(ctx, id.ResourceGroup, id.Name, nil)
		if err != nil {
