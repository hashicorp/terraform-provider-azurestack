...
func (id ResourceIdGenerator) TestCode() string {
	...
	if id.TestPackageSuffix != "" {
		importLine = fmt.Sprintf("\"github.com/hashicorp/terraform-provider-azurestack/internal/services/%s/parse\"", id.ServicePackageName)
	}
...
import (
	...
	"github.com/hashicorp/terraform-provider-azurestack/internal/services/%[2]s/parse"
)
...
import (
	...
	"github.com/hashicorp/terraform-provider-azurestack/internal/services/%[4]s/validate"
)
...
func goFmtAndWriteToFile(filePath, fileContents string) error {
	...
	if err := os.WriteFile(filePath, []byte(*fmt), 0o644); err != nil { // nolint:gosec
		return err
	}
