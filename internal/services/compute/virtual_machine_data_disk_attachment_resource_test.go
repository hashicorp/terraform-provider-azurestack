...
func TestAccVirtualMachineDataDiskAttachment_basic(t *testing.T) {
	data := acceptance.BuildTestData(t, "azurestack_virtual_machine_data_disk_attachment", "test")
...
func TestAccVirtualMachineDataDiskAttachment_requiresImport(t *testing.T) {
	...
	data.ResourceTest(t, r, []acceptance.TestStep{
		...
		},
		{
			...
			ExpectError: acceptance.RequiresImportError("azurestack_virtual_machine_data_disk_attachment"),
		},
	})
}
...
func TestAccVirtualMachineDataDiskAttachment_multipleDisks(t *testing.T) {
	data := acceptance.BuildTestData(t, "azurestack_virtual_machine_data_disk_attachment", "first")
	r := VirtualMachineDataDiskAttachmentResource{}

	secondResourceName := "azurestack_virtual_machine_data_disk_attachment.second"
...
resource "azurestack_virtual_machine_data_disk_attachment" "import" {
  managed_disk_id    = azurestack_virtual_machine_data_disk_attachment.test.managed_disk_id
  virtual_machine_id = azurestack_virtual_machine_data_disk_attachment.test.virtual_machine_id
  lun                = azurestack_virtual_machine_data_disk_attachment.test.lun
  caching            = azurestack_virtual_machine_data_disk_attachment.test.caching
}
`, r.basic(data))
}
...
resource "azurestack_virtual_machine_data_disk_attachment" "first" {
  managed_disk_id    = azurestack_managed_disk.test.id
  virtual_machine_id = azurestack_virtual_machine.test.id
  lun                = "10"
  caching            = "None"
...
resource "azurestack_managed_disk" "second" {
  name                 = "%d-disk2"
  location             = azurestack_resource_group.test.location
  resource_group_name  = azurestack_resource_group.test.name
  storage_account_type = "Standard_LRS"
...
resource "azurestack_virtual_machine_data_disk_attachment" "second" {
  managed_disk_id    = azurestack_managed_disk.second.id
  virtual_machine_id = azurestack_virtual_machine.test.id
  lun                = "20"
  caching            = "ReadOnly"
}
