...
layout: "azurestack"
page_title: "Azure Resource Manager: azurestack_key_vault"
description: |-
  Manages a Key Vault.
---
...
~> **Note:** Considering Azure Stack Hub (version 2108) limiations, soft delete feature is not yet supported.

~> **Note:** It's possible to define Key Vault Access Policies both within [the `azurestack_key_vault` resource](key_vault.html) via the `access_policy` block and by using [the `azurestack_key_vault_access_policy` resource](key_vault_access_policy.html). However it's not possible to use both methods to manage Access Policies within a KeyVault, since there'll be conflicts.
...
* `sku_name` - (Required) The Name of the SKU used for this Key Vault. Possible values is `standard`.
...
-> **NOTE** Since `access_policy` can be configured both inline and via the separate `azurestack_key_vault_access_policy` resource, we have to explicitly set it to empty slice (`[]`) to remove it.
