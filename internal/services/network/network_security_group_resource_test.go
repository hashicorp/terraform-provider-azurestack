...
func TestAccNetworkSecurityGroup_requiresImport(t *testing.T) {
	...
	data.ResourceTest(t, r, []acceptance.TestStep{
		...
		},
		{
			...
			ExpectError: acceptance.RequiresImportError("azurestack_network_security_group"),
		},
	})
}
...
resource "azurestack_network_security_group" "import" {
  name                = azurestack_network_security_group.test.name
  location            = azurestack_network_security_group.test.location
  resource_group_name = azurestack_network_security_group.test.resource_group_name
}
`, r.basic(data))
}
