	...
	return func(ctx context.Context, d *pluginsdk.ResourceData, meta interface{}) (data []*pluginsdk.ResourceData, err error) {
		...
		if profile := vm.VirtualMachineProperties.StorageProfile; profile != nil {
			...
			if profile.OsDisk.Vhd != nil {
				return []*pluginsdk.ResourceData{}, fmt.Errorf("The %q resource only supports Managed Disks - please use the `azurestack_virtual_machine` resource for Unmanaged Disks", resourceType)
			}
		}
		...
		if vm.VirtualMachineProperties.OsProfile == nil {
			return []*pluginsdk.ResourceData{}, fmt.Errorf("The %q resource doesn't support attaching OS Disks - please use the `azurestack_virtual_machine` resource instead", resourceType)
		}
